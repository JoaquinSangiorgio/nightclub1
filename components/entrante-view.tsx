...
export default function EntranteView() {
  ...
  const loadData = async () => {
    try {
      ...
      // Solo mostramos insumos base (tipo botella) para ingresar stock
      setBottles(bottlesData.filter(b => b.tipo === 'botella'))
      // Tipamos y filtramos alertas no leídas
  ...
  const addToCart = (bottle: Botella) => {
    ...
    if (!exists) {
      ...
      toast.success(`${bottle.nombre} en lista de ingreso`)
    }
  }
  ...
  const handleFinalize = async () => {
    ...
    const toastId = toast.loading("Actualizando stock real en mililitros...")
    setIsSaving(true)
  ...
  return (
    <div className="h-[calc(100vh-140px)] flex flex-col lg:flex-row gap-8 font-rounded animate-in fade-in duration-500 overflow-hidden">
                ...
                {alerts.map(alert => (
                  <div key={alert.id} className="flex-shrink-0 bg-rose-500/10 border border-rose-500/30 p-3 rounded-2xl flex items-center gap-4 animate-in slide-in-from-left-4">
                    <div className="min-w-0">
                      ...
                      <p className="text-[8px] text-rose-400 font-bold uppercase">Reposición Urgente</p>
                    </div>
                    <button onClick={() => handleMarkAsRead(alert.id)} className="p-1.5 bg-rose-500 text-white rounded-lg hover:bg-rose-400 transition-colors">
      ...
      {/* PANEL DERECHO: MANIFIESTO (CARRITO) */}
      <div className="flex-1 bg-slate-900/40 border-2 border-slate-800 rounded-[3rem] flex flex-col overflow-hidden shadow-2xl relative">
        <div className="p-8 border-b-2 border-slate-800 bg-emerald-500/5 flex items-center justify-between">
          ...
          {cart.length > 0 && <span className="text-[10px] font-black bg-emerald-500 text-black px-3 py-1 rounded-full uppercase italic">{cart.length} SKUs</span>}
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
          {cart.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-slate-700 opacity-20">
              ...
              <p className="font-black italic text-xs uppercase tracking-[0.3em] text-center leading-relaxed">Esperando<br/>Nuevos Packs</p>
            </div>
          ) : (
            cart.map((item) => (
              <div key={item.id} className="bg-slate-950/80 p-5 rounded-[2.5rem] border border-slate-800 space-y-4 animate-in zoom-in-95">
                <div className="flex justify-between items-start px-2">
                  ...
                  <button onClick={() => setCart(cart.filter(i => i.id !== item.id))} className="p-2 bg-rose-500/10 text-rose-500 rounded-xl hover:bg-rose-500 transition-all">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2 text-center">
                    <label className="text-[9px] font-black text-slate-600 uppercase tracking-widest">Botellas</label>
                    <input 
                      type="number" 
                  ...
                  <div className="space-y-2 text-center">
                    <label className="text-[9px] font-black text-slate-600 uppercase tracking-widest">Costo Unitario</label>
        ...
        {cart.length > 0 && (
          <div className="p-8 bg-slate-900 border-t-2 border-slate-800 space-y-6">
            <div className="relative group">
              ...
              <input 
                type="text" placeholder="Notas de recepción (Proveedor / Factura)..." value={nota}
            ...
            <button onClick={handleFinalize} disabled={isSaving} className={`w-full py-6 ${isSaving ? 'bg-slate-800 text-slate-600' : 'bg-emerald-600 hover:bg-emerald-500 shadow-emerald-600/10'} text-white font-black rounded-[2rem] flex items-center justify-center gap-3 shadow-2xl active:scale-95 text-xl uppercase italic transition-all`}>
              ...
              {isSaving ? 'Actualizando...' : 'Confirmar Recepción'}
            </button>
          </div>
        )}
      </div>
