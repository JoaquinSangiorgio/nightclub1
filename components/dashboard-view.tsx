...
export function DashboardView({ onNavigate }: DashboardViewProps) {
  ...
  {[
    { 
      ...
      val: stats?.totalBotellas || 0, 
      icon: Package, 
      color: 'text-indigo-400' 
    },
    { 
      label: 'Valorización', 
      val: `$${(stats?.valorTotal || 0).toLocaleString()}`, 
      icon: DollarSign, 
      color: 'text-emerald-400' 
    },
    { 
      label: 'Alertas Stock', 
      val: stats?.conteoStockBajo || 0, 
      icon: AlertTriangle, 
      color: 'text-orange-400' 
    },
    { 
      label: 'Agotados', 
      val: stats?.conteoSinStock || 0, 
      icon: XCircle, 
      color: 'text-rose-400' 
    },
  ].map((s, i) => (
    <div key={i} className="bg-[#0f172a]/40 border-2 border-slate-800 p-6 rounded-[2rem] hover:border-slate-700 transition-all group relative overflow-hidden">
      {/* Icono de fondo decorativo para darle ese toque "Premium" */}
      <s.icon className={`absolute -right-2 -bottom-2 w-16 h-16 opacity-5 ${s.color}`} />
